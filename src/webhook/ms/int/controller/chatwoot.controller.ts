...
export class ChatwootController {
  ...
  handleConversationCreated(@Body() payload: ChatwootWebhookPayload) {
  ...
  @Post('MessageCreated')
  handleMessageCreated(@Body() payload: ChatwootWebhookPayload) {
    const channel = payload.channel.split('::')[1];
    const contact_id = payload.meta.sender.id;
    const conversation_id = payload.id;
    const inbox_id = payload.inbox_id;
    ...
    //this.logger.log('Chatwoot Message Created:', JSON.stringify(data));
